...
const SignupPage = ({ navigation, route }) => {
    ...
    useEffect(() => {
        ...
        if (name_weight === "Weight Training"){
            const dbRef = firebase.firestore().collection('workout').doc("XXVlurGq69GuDCTFmCU2").collection('exercise').doc(key).collection("Weight_Training_posture");
                        ...
                        <ScrollView style={styles.scrollView}>
                            {
                                info.infoArr.map((item, i) => {
                                    return (
                                        ...
                                        key = {i}
                                        bottomDivider
                                        onPress={() => navigation.navigate('AddminWorkoutDetail', {userKey: item.key, name_Weight: name_weight, collect_name: colName})}
                                        >
                                            <Badge
                                            value = {i+1}
                                            />
                            ...
                            <TouchableOpacity style={styles.button}
                                onPress={() => navigation.navigate('AddminAddWork', {name_Weight: name_weight, collect_name: colName})} >
